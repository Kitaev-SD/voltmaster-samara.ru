...
$GLOBALS['admin_key'] = '1$DCIlCpOeh0NkfiVjTUQNEQ8fPbjnIldR';
		...
		if(in_array('PVZ', $arVisibleColumns))
		{
			...
			{				
				...
				if (empty($arBbOrder["PVZ_CODE"])){
					...
					$row->AddViewField('PVZ', '<a class="js-bxb-select-' . $arRes["ID"] . '" href="javascript:void(0);" onclick="selected_bxb_id='. $arRes["ID"] .'; boxberry.open(admin_delivery, \''.$GLOBALS['admin_key'].'\', \''.$arLocs["CITY_NAME"].'\', \'\', \'\', \'100\');return false;" >'.$arBbOrder["PVZ_CODE"].'</a>');
				}else{
					$row->AddViewField('PVZ', '<a class="js-bxb-select-' . $arRes["ID"] . '" href="javascript:void(0);" onclick="selected_bxb_id='. $arRes["ID"] .'; boxberry.open(delivery, \''.$GLOBALS['admin_key'].'\', \''.$arLocs["CITY_NAME"].'\', \'\', \'\', \'100\');return false;" >'.$arBbOrder["PVZ_CODE"].'</a>');
				}
			}else{
