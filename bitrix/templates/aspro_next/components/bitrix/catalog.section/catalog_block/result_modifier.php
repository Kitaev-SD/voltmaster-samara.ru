...
while ($data = $result->fetch()){
    foreach($data['ACTIONS_LIST']['CHILDREN'] as $SaleCond){
        foreach($SaleCond['CHILDREN'] as $ItemCond){
        	if ($ItemCond['CLASS_ID']=="CondBsktFldQuantity" && $ItemCond['DATA']['logic']=="EqGr") {
                $SaleQuantities[]= $ItemCond['DATA']['value'];
            }
		}
	}
}
$UniqueSaleQuantities=array_unique($SaleQuantities);
...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$arItem['LAST_ELEMENT'] = 'N';
       	/*Вычисление всех скидок для товара*/
        $arResult['ITEMS'][$key]["CUR_DISCONTS"] = [];
        $cur_id = $arItem['ID'];
        $start_quant = 1;
        $end_quant = 120;
        if($arItem["CATALOG_QUANTITY"] >= $start_quant ){$dostart = true;};
		$ItemSaleQuantity=[];
        if($dostart) {

            	foreach ($UniqueSaleQuantities as $cur_quant) {
					if($cur_quant<=$arItem['CATALOG_QUANTITY']) {
                        $tmp_disc_arr = check_discond($cur_id, $cur_quant);
                        $ItemSaleQuantity[] = $tmp_disc_arr['PRICE_ONE'];
                    }
            }
        }


        if (!empty($ItemSaleQuantity)&&min($ItemSaleQuantity)<$arItem['MIN_PRICE']['VALUE']){
            $arItem['MIN_PRICE_NEW']=min($ItemSaleQuantity);
        	$arItem['MIN_PRICE']['VALUE'] = min($ItemSaleQuantity);
            $arItem['MIN_PRICE']['DISCOUNT_VALUE'] = min($ItemSaleQuantity);
            $arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE'] = min($ItemSaleQuantity).' руб.';
