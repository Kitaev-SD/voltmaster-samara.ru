		...
		<div class="info_item">
			...
			<?if($isArticle || $arResult["BRAND_ITEM"] || $arParams["SHOW_RATING"] == "Y" || strlen($arResult["PREVIEW_TEXT"])){?>
				<div class="top_info">
                    ...
                    <?if(strlen($arResult["PROPERTIES"]["PROIZVODITEL"]["VALUE"])):?>
                        <div class="preview_text dotdot">Производитель: <span><?=$arResult["PROPERTIES"]["PROIZVODITEL"]["VALUE"]?></span></div>
                    <?endif;?>
				</div>
			<?}?>
					...
					<div class="cost prices clearfix">
						...
						<?}else{?>
							...
							if(isset($arResult['PRICE_MATRIX']) && $arResult['PRICE_MATRIX']) // USE_PRICE_COUNT
							{
								...
								<?endif;?>
                                <? if(!empty($arResult["CUR_DISCONTS"])){ ?> <div class="count_price">Цена за единицу товара</div>  <? } ?>

                        <?=CNext::showPriceMatrix($arResult, $arParams, $strMeasure, $arAddToBasketData);?>

                        <?/* if(!empty($arResult["CUR_DISCONTS"])){ ?>
                            <script>
								...
								<div class = "tax_prices_block"><?
                                    foreach ($arResult["CUR_DISCONTS"] as $key => $cur_disc_item){   ?>
                                        <script>
                                               ...
                                               disconts_arr__price_one.push("<?=$cur_disc_item['PRICE_ONE']?>");
                                               disconts_arr__price_total.push("<?=$cur_disc_item['PRICE_ALL']?>");
                                               disconts_arr__discont_one.push("<?=$cur_disc_item['DISCOUNT_ONE']?>");
                                               disconts_arr__discont_total.push("<?=$cur_disc_item['DISCOUNT_PRICE_ALL']?>");
                                        </script>
                                        <div class="tax_prices_item">
                                            <div class="count_price">Цена при покупке от <?= $cur_disc_item['QUANTITY_ALL']?> <?=$arResult['measure'];?></div>
                                            <div class="price_matrix_block">
                                                <div class="price_matrix_wrapper disc_price disc_price_<?=$key?>">
                                                    <div class="price" data-currency="RUB" data-value="<?php echo round($cur_disc_item['PRICE_ONE']*100)/100 ; ?>">
                                                        <span><span class="values_wrapper"><span class="price_value"><?php echo number_format( (round($cur_disc_item['PRICE_ONE']*100)/100 ), 2, ".", ' ');?></span><span class="price_currency"> руб.<?if($arResult['measure']):?>/<?=$arResult['measure'];?><?endif;?></span></span></span>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="quantity_block_wrapper">
                        ...
                        <? if(!empty($arResult["PROPERTIES"]["FILES"]["VALUE"])) {
                            foreach ($arResult["PROPERTIES"]["FILES"]["VALUE"] as $files) {
                                $filePath = CFile::GetPath($files);
                                $rsFile = CFile::GetByID($files);
                                $arFile = $rsFile->Fetch();
                                $fileSize = CFile::FormatSize($arFile["FILE_SIZE"]); ?>
                                <div class="link-download">
                                    <a href="<?=$filePath?>" target="_blank"><span class="icon">PDF</span> Документация</a>
                                    <span class="gray"><?=$fileSize?></span>
                                </div>
                        <? }
                            unset($files);
                        } ?>
