	...
	for ($cur_quant = $start_quant; $cur_quant <= $end_quant; $cur_quant++) {
		...
		if (
			...
			if (empty($arResult["CUR_DISCONTS"])) {
				$arResult["CUR_DISCONTS"][] = $tmp_disc_arr;
			} elseif (Math.round($arResult["CUR_DISCONTS"][  count($arResult["CUR_DISCONTS"]) - 1]['DISCOUNT_ONE']) != Math.round($tmp_disc_arr['DISCOUNT_ONE'])) {
				$arResult["CUR_DISCONTS"][] = $tmp_disc_arr;
			}
		}
	}
...
$arResult['measure'] = $arMeasure[$arResult['ID']]['MEASURE']['SYMBOL_RUS'];
