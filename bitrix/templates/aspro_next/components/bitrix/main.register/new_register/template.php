...
<div class="module-form-block-wr registraion-page">
	...
	<?}else{?>
		<div class="form-block border_block">
			<div class="wrap_md">
				<div class="main_info iblock">
					...
					<form id="registraion-page-form" method="post" action="<?=POST_FORM_ACTION_URI?>" name="regform" enctype="multipart/form-data" >
						...
						###Сортировка вывода полей, чтобы выводились в определенном порядке
						unset($arFields);
						$arFields = array ( 'NAME' => 'NAME', 'EMAIL' => 'EMAIL', 'LOGIN' => 'LOGIN', 'PASSWORD' => 'PASSWORD', 'CONFIRM_PASSWORD' => 'CONFIRM_PASSWORD');
						?>
						<p><span style="margin-right: 50px;"><input type="radio" style="width: 20px;" checked name="type_user" id="fiz" value="fiz"><?=GetMessage("TYPE_USER_FIZ");?></span>
							<span><input type="radio" style="width: 20px;" name="type_user" id="uri" value="uri"><?=GetMessage("TYPE_USER_URI");?></span></p>
						...
						<?if($arUFields){?>
								...
								<div class="r <?if ($k != "UF_CHECKORG"):?>organization<?endif;?> <?if ($k == "UF_INN" || $k == "UF_ORGNAME"):?>orgreq<?endif;?>" style="display:none;">
									<label><?=$arUField["EDIT_FORM_LABEL"];?>:<?if ($k == "UF_INN" || $k == "UF_ORGNAME"):?><span class="star"> *</span><?endif;?></label>
									<?$APPLICATION->IncludeComponent(
						...
						<div class="form-control bg register-captcha captcha-row clearfix">
							<div class="iblock label_block">
								<label><span><?=GetMessage("REGISTER_CAPTCHA_YEAR")?>&nbsp;<span class="star">*</span></span></label>
								<div class="captcha_image">
                                    <div id="html_element_register">
	                                    <input type="number" name="yearcheck" value="" required/>
                                    </div>
								</div>
							</div>
							<div class="iblock text_block"></div>
						...
						<div class="but-r">
							...
							<?endif;?>
							<button class="btn btn-default short" type="submit" id="send_register" name="register_submit_button1" value="<?=GetMessage("AUTH_REGISTER")?>">
			...
			<div class="social_block iblock" id="infoForAll">Зарегистрированные пользователи получают скидку 3% на электронные компоненты. При первом заказе от 3000 руб. мы приложим к вашему заказу дисконтную карту.</div>
			<div class="social_block iblock organization" id="infoForOrg" style="display:none;">При регистрации как юридическое лицо вам будет доступна оплата безналичным способом со счёта компании, но при этом вы сможете оплатить любой заказ как физическое лицо (за наличные, через платёжные системы или пластиковой картой).</div>
