...
$arFilter = array('ACTIVE' => 'Y', 'IBLOCK_ID' => $arParams['IBLOCK_ID'], "CODE" => $arResult["VARIABLES"]["SECTION_CODE"]);
$dbRes = CIBlockSection::GetList(array('SORT'=>'ASC'), $arFilter, false, Array("UF_SECTADDDESCR"));

while ($arRes = $dbRes->GetNext())
{
    $UF_SECTADDDESCR = $arRes['UF_SECTADDDESCR'];
}
if(strlen($UF_SECTADDDESCR)>1) {
    echo "<div id='sect_descript_add' class='sect_descript_add'>".$UF_SECTADDDESCR."</div>";
}
...
<div class="js_wrapper_items" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arTransferParams, false))?>'>
    <? $APPLICATION->IncludeFile(
        SITE_DIR . 'include/search_in_catalog_section.php',
        [
            'FORM_ACTION' => $APPLICATION->GetCurPage(),
            'SECTION_ID' => $arSection['ID'],
            'DEFAULT_VALUE' => htmlspecialchars($_REQUEST['search_query']),
            // TODO вынести в lang-файлы
            'SEARCH_INPUT_PLACEHOLDER' => 'Поиск по разделу',
            'SEARCH_SUBMIT_NAME' => 'Найти',
