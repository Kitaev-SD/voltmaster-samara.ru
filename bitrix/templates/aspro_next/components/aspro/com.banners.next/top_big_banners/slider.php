...
<?
function isElemActiveDate($arItem_id){
	$bool = true;
	if (CModule::IncludeModule("iblock")) {
		$res = CIBlockElement::GetByID($arItem_id);
		
		if($ar_res = $res->GetNext()){
			$site_format = CSite::GetDateFormat();
			$DATE_ACTIVE_FROM = MakeTimeStamp($ar_res['DATE_ACTIVE_FROM'], $site_format);
			$DATE_ACTIVE_TO = MakeTimeStamp($ar_res['DATE_ACTIVE_TO'], $site_format);
			$CURRENT_DATE = (new DateTime())->getTimestamp();
		}

		if(empty($DATE_ACTIVE_FROM) && empty($DATE_ACTIVE_TO)) {
			$bool = true;
		} else if(!empty($DATE_ACTIVE_FROM) && empty($DATE_ACTIVE_TO)) {
			if($CURRENT_DATE >= $DATE_ACTIVE_FROM) {
				$bool = true;
			} else {
				$bool = false;
			}
		} else if(empty($DATE_ACTIVE_FROM) && !empty($DATE_ACTIVE_TO)) {
			if($CURRENT_DATE <= $DATE_ACTIVE_TO) {
				$bool = true;
			} else {
				$bool = false;
			}
		} else if(!empty($DATE_ACTIVE_FROM) && !empty($DATE_ACTIVE_TO)) {
			if($CURRENT_DATE >= $DATE_ACTIVE_FROM && $CURRENT_DATE <= $DATE_ACTIVE_TO) {
				$bool = true;
			} else {
				$bool = false;
			}
